...
表示サイズ「タスクトレイ」用のディスクアクセス LED。`.ico`（16×16 と 32×32 を含む、32bpp+アルファ）を想定。`TAssetStore` の PNG/BMP キャッシュは使わず、`Shell_NotifyIcon` 向けに直接読み込む。コンパクト/フルによらず1つだけ（アイコンでありメーター配置とは無関係なので分けない）。
