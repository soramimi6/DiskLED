...
進捗（すべて `feature/3.2.0` 上・`master` 未マージ）: 項目 **1〜17 がすべて完了**（項目2はベゼル質感の作り直しを対象外として見送った上での完了、表の「ステータス」列参照）。項目 **18・19 は実装済み・実機確認待ち**（それぞれ `work/3.2.0-18-power-remain-time-format`、`work/3.2.0-19-options-tab-top-margin`）。公開ドキュメント（`USAGE`/`FEATURES`/`NOTES`/`CHANGELOG` の JA+EN）は3.2.0の内容を反映済み。
...
| 18 | ダッシュボード電源パネルの残時間表示を単位付きに改善（「1:1」→「1時間1分」） | 高（原因箇所を特定済み） | 低（`DrawPowerPanel` のフォーマット処理と文字列3件を追加） | **実装済み**（`work/3.2.0-18-power-remain-time-format`、実機確認待ち） | 低（表示改善） |
| 19 | オプション画面のタブ上部マージンを調整（タイトルバーに近すぎる） | 高（原因箇所を特定済み） | 低（`Padding.Top` を設定するのみ） | **実装済み**（`work/3.2.0-19-options-tab-top-margin`、実機確認待ち） | 低（表示改善） |
...
- フォームに `Padding.Top = 8` を設定（[uOptionsForm.dfm:15](../src/uOptionsForm.dfm#L15)）。`Padding` は `TWinControl.AdjustClientRect`（VCL 実ソース `Vcl.Controls.pas` で確認済み）が `Align` 対象の子コントロールへ渡すクライアント矩形に直接反映されるため、`PageControl1` の座標計算やアンカー設定に一切手を入れずにタブ帯を8px下げられる。
- `PageControl1` は `Padding.Top` 分だけ自動的に縮むため、タブ本体・下部ボタン行（`PnlButtons`、`Align = alBottom`）の高さを従来どおり保つよう `ClientHeight` を 470→478（[uOptionsForm.dfm:7](../src/uOptionsForm.dfm#L7)）に広げて相殺した。ダイアログが縦に8px大きくなる以外、既存レイアウトへの影響はない。

### 実装後に実機で見ること

- オプション画面を開き、タブ帯（General/Display/Tray LED/Ping・ネットワーク）とタイトルバーの間に目視で分かる隙間があること。
- 各タブ内のコントロール（特に縦方向にスペースが厳しい箇所）が既存どおり収まっており、はみ出し・重なりが無いこと。
- 125%/150%/200% の各 DPI でも隙間が縮まったり潰れたりしないこと（`Scaled = True` の標準 VCL スケーリングに乗るだけなので通常は問題ないはずだが確認する）。
