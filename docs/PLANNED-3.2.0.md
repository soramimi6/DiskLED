...
項目 **25** はユーザー報告により判明した、「常に手前に表示」有効時にガジェット本体の右クリックメニューが本体の背面に回り込む現象の修正。項目 16（最前面の再主張）と関係が深い。着手順・3.2.0 出荷の必須条件には含めない（項目 16〜24 と同様の事後対応枠）。
...
進捗（すべて `feature/3.2.0` 上・`master` 未マージ）: 項目 **1〜20 がすべて完了**（項目2はベゼル質感の作り直しを対象外として見送った上での完了、表の「ステータス」列参照）。項目 **21〜25 もすべて完了**。公開ドキュメント（`USAGE`/`FEATURES`/`NOTES`/`CHANGELOG` の JA+EN）は3.2.0の内容を反映済み。
...
| 25 | 「常に手前に表示」有効時、ガジェット本体の右クリックメニューが本体の背面に回り込む現象を修正 | 高（原因箇所を特定済み・実機で再現確認済み） | 低（`PollStayOnTop` にメニューモード判定を追加） | **完了**（PR #45、実機確認済み） | 高（不具合修正） |
...
`PollStayOnTop` の先頭ガードに、`GetGUIThreadInfo(GetCurrentThreadId, …)` の `flags` が `GUI_INMENUMODE` または `GUI_POPUPMENUMODE`（RTL の `Winapi.Windows.pas` に定義済み）を含む間は再実行をスキップする判定を追加した（[uMainForm.pas:865-875](../src/uMainForm.pas#L865)）。自スレッドのどのメニュー（ガジェット・トレイ・ダッシュボード）にも同じ判定で効き、メニューの開閉をフラグで管理する必要がない。メニューが閉じた次のティックで従来どおり再実行するため、項目16の目的（他の最前面窓に押し出された本体が2秒程度で戻る）は保たれる。

### 実機確認結果

- 「常に手前に表示」有効で、本体を右クリックしたメニューを開いたままにしても、本体がメニューを隠さないことを確認済み。トレイアイコンのメニューでも同様。
- 項目16の目的（他の最前面窓に押し出された本体が2秒程度で最前面に戻る）が損なわれていないことを確認済み。
