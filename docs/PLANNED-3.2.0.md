...
進捗（2026-09-11 時点、すべて `feature/3.2.0` 上・`master` 未マージ）: 項目 **1・3・4・7** は実装＋IDE ビルド検証済み（項目 7 は完了。項目 4 は Disk/Net カードの DPI 確認のみ残る、表の「ステータス」列参照）。項目 5・6 は設計判断を確定（コミット `fa4a1c9`）済みで実装未着手。項目 2 は保留。残りの着手順は `5 → 6`。
