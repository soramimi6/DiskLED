...
項目 **25** はユーザー報告により判明した、「常に手前に表示」有効時にガジェット本体の右クリックメニューが本体の背面に回り込む現象の対応策を検討するタスク。項目 16（最前面の再主張）と関係が深い。
...
進捗（すべて `feature/3.2.0` 上・`master` 未マージ）: 項目 **1〜20 がすべて完了**（項目2はベゼル質感の作り直しを対象外として見送った上での完了、表の「ステータス」列参照）。項目 **21〜24 もすべて完了**。項目 **25 は未着手**（対応策の検討）。公開ドキュメント（`USAGE`/`FEATURES`/`NOTES`/`CHANGELOG` の JA+EN）は3.2.0の内容を反映済み。
...
| 25 | 「常に手前に表示」有効時、ガジェット本体の右クリックメニューが本体の背面に回り込む現象の対応策を検討 | 高（原因の見立てを特定済み、再現確認が必要） | 低 | **未着手**（対応策の検討） | 高（不具合修正） |
...
## 25. 「常に手前に表示」有効時にガジェット本体の右クリックメニューが背面に回り込む現象の対応策を検討

ユーザー報告により判明。「常に手前に表示」を有効にしたガジェット本体で右クリックメニュー（トレイアイコンのメニューを含む）を開くと、メニューが本体ウィンドウの背面に回り込んで見づらくなる。

### 原因の見立て（実ソース確認済み。実機での再現確認は未実施）

- 項目16で追加した `PollStayOnTop`（[uMainForm.pas:842](../src/uMainForm.pas#L842)）は、フレームタイマー（`TimerTick`、[uMainForm.pas:1092](../src/uMainForm.pas#L1092)）から2秒間隔で本体に `SetWindowPos(HWND_TOPMOST, …)` を再実行する。先頭のガード（[uMainForm.pas:861-863](../src/uMainForm.pas#L861)）はオプション画面・ダッシュボード・Tracert 窓が開いている間しか再実行を止めておらず、**ポップアップメニューが開いている間は止めていない**。
- ガジェットのメニューは `TPopupMenu.Popup`（[uMainForm.pas:1713](../src/uMainForm.pas#L1713) の `ShowAppPopup`）で、トレイアイコン側も VCL の `TTrayIcon` が同じ `PopupMenu.Popup` を呼ぶ（VCL 実ソース `Vcl.ExtCtrls.pas` の `TTrayIcon`）。`TPopupMenu.Popup` は `TrackPopupMenu`（VCL 実ソース `Vcl.Menus.pas:5058`、所有ウィンドウは VCL の非表示ウィンドウ `PopupList.Window`）を呼んでメニューを閉じるまで戻らないモーダルループに入るが、その間も `WM_TIMER` は配送され `TimerTick` → `PollStayOnTop` が動く。再実行された本体は、最前面帯の中でメニューウィンドウ（同じく最前面帯）より上に出るため、メニューの上に被さる。メニューを開いてから最大2秒後に発生する見込み。
- 再現確認のポイント: メニューを開いた直後は正常で、最大2秒以内に本体がメニューを隠す挙動であれば上記の見立てで確定。開いた瞬間から背面に出る場合は別原因（`PopupList.Window` が最前面帯に入っていない等）を疑う。

### 対応案

1. **メニューモード中は再実行しない（推奨）**: `PollStayOnTop` の先頭ガードに `GetGUIThreadInfo(GetCurrentThreadId, …)` の `flags` の `GUI_INMENUMODE`（RTL の `Winapi.Windows.pas` に `GUI_INMENUMODE = $00000004` と `GetGUIThreadInfo` が定義済み）判定を追加する。自スレッドのどのメニュー（ガジェット・トレイ・ダッシュボード等）でも同じ判定で済み、メニューの開閉をフラグ管理する必要がない。変更は数行で、2秒間隔の再実行という項目16の仕組みはそのまま残る。
2. **開閉フラグ方式**: `ShowAppPopup` の `FPopup.Popup` 呼び出しの前後でフラグを立てる。ただしトレイ側は `TTrayIcon` 内部が `Popup` を呼ぶため囲めず、`OnMouseUp` 等を自前で処理してメニュー表示を置き換える必要があり、変更が大きい。
3. **「上に外来の最前面窓がいるときだけ再実行」方式**: `GetWindow(Handle, GW_HWNDPREV)` で本体より上の最前面窓をたどり、メニュー（クラス `#32768`）や自分の所有ウィンドウを除いた外来の最前面窓がいる場合だけ `SetWindowPos` する。メニューだけでなく、項目16のレビューで指摘された「所有ウィンドウを手書きで列挙するガード」も不要にできるが、実装と確認が最も重く、外来ウィンドウの判定（IME ウィンドウ等）に落とし穴がある。

### 実装時に実機で見ること

- 「常に手前に表示」有効で、本体を右クリックしたメニューを5秒以上開いたままにしても、本体がメニューを隠さないこと。トレイアイコンのメニューでも同様。
- 項目16の目的（他の最前面窓に押し出された本体が2秒程度で最前面に戻る）が損なわれていないこと。
