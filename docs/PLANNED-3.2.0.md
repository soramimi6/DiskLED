...
進捗（すべて `feature/3.2.0` 上・`master` 未マージ）: 項目 **1〜17・19 がすべて完了**（項目2はベゼル質感の作り直しを対象外として見送った上での完了、表の「ステータス」列参照）。項目 **18 は実装済み・実機確認待ち**（`work/3.2.0-18-power-remain-time-format`）。公開ドキュメント（`USAGE`/`FEATURES`/`NOTES`/`CHANGELOG` の JA+EN）は3.2.0の内容を反映済み。
