...
進捗（すべて `feature/3.2.0` 上・`master` 未マージ）: 項目 **1〜17・19・20 がすべて完了**（項目2はベゼル質感の作り直しを対象外として見送った上での完了、表の「ステータス」列参照）。項目 **18 は実装済み・実機確認待ち**（`work/3.2.0-18-power-remain-time-format`）。公開ドキュメント（`USAGE`/`FEATURES`/`NOTES`/`CHANGELOG` の JA+EN）は3.2.0の内容を反映済み。
...
| 20 | タスクトレイ LED アイコンのネットグリフのデザイン調整 | 高（原因箇所を特定済み） | 低（`generate-tray-icons.ps1` の色・線幅を2箇所調整） | **実装済み**（`work/3.2.0-20-tray-net-off-glyph-dim`、プレビュー画像でユーザー確認済み。Delphi コード変更なしのため IDE ビルド不要） | 低（表示改善） |
