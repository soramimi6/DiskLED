...
進捗（すべて `feature/3.2.0` 上・`master` 未マージ）: 項目 **1・3・4・5・6・7・8・9 が完了**（項目 4 は Disk/Net カードの DPI 確認のみ残る、表の「ステータス」列参照）。項目 2 は保留。公開ドキュメント（`USAGE`/`FEATURES`/`NOTES`/`CHANGELOG` の JA+EN）も 3.2.0 分を反映済み。
