...
3.2.0 の対象は **1・3・4・5・6・7・8・9**（項目 2 は着手順から外した保留枠）。着手順は `7 → 1 → 3 → 4 → 5 → 8 → 9 → 6`。項目 7 は不具合修正のため最優先で着手する。小規模で自己完結する 1 で 3.2.0 の作業フローを慣らし、文字列基盤（3）・動的 PDH カウンタ（4）という基盤性のある項目を先に据えてから重い項目へ進む。項目 8（オプション画面の複数ページ化）は項目 5 でオプション画面の設定項目が増えた後、asset-editor（6）の前に片付ける。項目 9（Info Bar スキンの拡張）も layout.cfg の形が変わるため、asset-editor（6）が扱う最終的な layout.cfg 形式に含める必要があり、6 の直前に置く。項目 6 は 3.2.0 最大の成果物で、layout.cfg 形式が項目 5 の `[Tray]` 撤去・項目 9 の Info Bar 拡張の後に確定するため最後に置く。各項目の相対的な優先度は表の「優先度」列を参照。

進捗（すべて `feature/3.2.0` 上・`master` 未マージ）: 項目 **1・3・4・5・7・8 が完了**（項目 4 は Disk/Net カードの DPI 確認のみ残る、表の「ステータス」列参照）。項目 9 は第一段階（コンパクト→フル複製）のみ実施済み。残るは項目 6（asset-editor、設計判断を確定済み・実装未着手）と項目 9 の第二段階（コンパクト側の情報整理）。項目 2 は保留。

| # | 機能 | 実現可能性 | 難易度 | ステータス | 優先度 |
|---|---|---|---|---|---|
| 1 | メインウィンドウの表示倍率をユーザー選択制にする | 高（拡大は `FScale100` 1 変数に集約済み） | 低〜中（ini キー＋メニュー＋`FScale100` の導出変更） | **実装済**（`feature/3.2.0`、IDE ビルド検証済。ライブなモニタ倍率変更時のリサイズ含む。公開ドキュメント追記が残タスク） | 高 |
...
| 9 | Info Bar スキンの拡張（フル表示の追加＋コンパクトの絞り込み） | 高（`layout.cfg` 追記のみで既存コードは無改修） | 低（第一段階）〜中（第二段階、コンパクト用の新規画像素材が必要） | **一部実装**（`assets/infobar/layout.cfg` へコンパクトの内容をフルとして複製済み。コンパクト側を情報を絞ったデザインへ作り替える第二段階は未着手） | 中 |
...
## 9. Info Bar スキンの拡張（フル表示の追加＋コンパクトの絞り込み）

Info Bar は同梱スキンの中で唯一フル表示を持たない（`[GeneralFull]` が無い）。フル表示を追加し、あわせてコンパクト側は現行より表示密度を絞ったデザインへ作り替える。

### スコープ確定事項

- **第一段階（本ドキュメント追記と同時に実施済み）**: 現行コンパクトの内容をそのままフルへ複製する。パーツ配置・画像・数値は一切変更しない。まず「フルが動く状態」を作ってから絞り込みを検討する順序。
- **第二段階（未着手）**: コンパクト側を、フルよりも情報を絞った新デザインへ作り替える。どの項目を削るか・配置をどう変えるかは未確定。**新規画像素材の作成が必要になる見込み**（現行素材はコンパクト/フル共用の前提で作られていないため）。
- フル側の情報を複製以上に拡張する（コンパクトに無い項目を新規に足す等）は本段階では行わない。

### 実施内容（`assets/infobar/layout.cfg` を確認済み）

- `[GeneralCompact]`（Width=531, Height=16, Transparent=0, MaskColor=#000000, Bg=`InfoBar_Base.png`）の内容をそのまま `[GeneralFull]` として複製した（[assets/infobar/layout.cfg:10-21](../assets/infobar/layout.cfg#L10)）。
- 各パーツの `...Compact` セクション（`Cpu`/`Mem`/`Swap`/`DiskReadMeter`/`DiskWriteMeter`/`NetInMeter`/`NetOutMeter`/`Ping`/`AudioL`/`AudioR`、計10種）をそれぞれ同内容の `...Full` として複製した。画像ファイル・`X`/`Y`/`Frames`/`Kind`/`Strength` は変更なし。`assets/LAYOUT.md` の「コンパクトとフルの両方に同じパーツを出したい場合は両方のセクションを書く（内容が同じでも構わない。同じ画像ファイルを指せば実素材は重複しない）」（[assets/LAYOUT.md:20](../assets/LAYOUT.md#L20)）という既定の使い方どおりで、Delphi 側のコード変更は不要。
- `uSkinLoader.pas` の `HasFull` 判定は `[GeneralFull]` の `Width`/`Height`/`Bg` が揃えば真になる（[uSkinLoader.pas:360-367](../src/view/uSkinLoader.pas#L360-L367)）。この複製により Info Bar でもダブルクリックでのコンパクト⇄フル切替、右クリックメニューのフル表示項目が有効になる（`FHasFull`、[uMainForm.pas:737](../src/uMainForm.pas#L737)）。

### 残タスク

- コンパクト側を情報を絞ったデザインへ作り替える第二段階（新規画像素材の作成を含む）。
- 現状フルは見た目・情報量ともにコンパクトと同一（531×16 のまま）。第二段階でコンパクトの構成が変わった後、フル側のサイズ・レイアウトを見直すかどうかも合わせて判断する。
- 公開ドキュメント（`USAGE.md`/`NOTES.md`/`FEATURES.md`/`CHANGELOG.md`/`CREDITS.md` の JA+EN）にある「Info Bar はコンパクトのみ」という記述（例 [public_docs/FEATURES.md:40](../public_docs/FEATURES.md#L40)）の是正は、第二段階でコンパクトのデザインが固まってから一括で行う。
