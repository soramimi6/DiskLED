...
DiskLED は単独メンテナ・GitHub ホスト（`soramimi6/DiskLED`）・CI 無しのプロジェクト。レビュー担当がいない代わりに、`/code-review` を通すことをレビューゲートとする。対象範囲・effort レベルの運用ルールは [REVIEW.md](../REVIEW.md) を参照（`work/...` 単位は `low`、`feature/x.y.z` へのバッチレビューは `medium`、リリース直前の最終レビューのみ `high`。`ultra` はコストが高いため本当に必要な場面でしか使わない）。
...
5. `/code-review`（effort レベルは [REVIEW.md](../REVIEW.md) を参照。`work/...` 単位なら `low`）を通し、指摘を反映する。個別に通さず `feature/x.y.z` へのバッチレビュー（`medium`）にまとめて回してもよい
