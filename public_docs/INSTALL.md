...
| 権限 | 標準ユーザーで動作（管理者不要） |
| 画面 | 第1版は表示スケール **100%** 前提（拡大時は OS 側の拡大に依存） |
...
| 用途 | 形態 | ファイル例 |
|------|------|------------|
| 紹介サイト・一般配布 | **インストーラー**（主配布） | `DiskLED_Setup_3.0.0.exe` |
| 上級者・開発試用 | **ポータブル zip** | `DiskLED-3.0.0-portable.zip` |
...
1. `DiskLED_Setup_3.0.0.exe` を実行する（管理者権限は不要）
...
1. `DiskLED-3.0.0-portable.zip` を任意のフォルダへ展開する
