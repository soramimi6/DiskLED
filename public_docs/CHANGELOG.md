...
- ダッシュボードは Windows のアプリのライト／ダークに合わせて配色する（設定の変更は開いている間にも反映）。表示中、左カラムのドーナツグラフは約 5 回／秒、数字・履歴・右カラムは 1 秒ごとに更新。履歴はメモリ上のみ（再起動で消える）。CPU / メモリ / SWAP の履歴は折れ線の下を薄い色で塗りつぶす（ディスク／ネットは重ね線のため対象外）
